...
if ($user==cesar91bo) {
    ...
    //para curar el contenido del msj...
    if ($contenido != "") {
    $contenido = filter_var($contenido, FILTER_SANITIZE_STRING);
    if ($contenido == "") {
        $errors .= 'Por favor ingrese un contenido válido.<br/><br/>';
    }
} else {
    $errors .= 'Por favor ingresar un contenido.<br/>';
}
        if ($contenido != ""){
...
header("Location: /RedSocial/muro.php");